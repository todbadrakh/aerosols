	  ...
	  if(`grep -ci "^$NEWNAME" list-done`) then
        ...
        cp -f ../$DIR/$FILE.xyz $NEWNAME.xyz
		  ...
		  if( $RERUN ) then
			  ...
			  echo "%mem=15gb" >> $NEWNAME.com
			  echo "%chk=j$NEWNAME.chk" >> $NEWNAME.com
			  echo "#T PW91PW91/6-311++g** geom(checkpoint,newdefinition) opt(maxcycles=500)" >> $NEWNAME.com
			  echo " " >> $NEWNAME.com
			  echo "$NEWNAME" >> $NEWNAME.com
			  echo " " >> $NEWNAME.com
			  echo "0 1" >> $NEWNAME.com
			  ...
			  sed -e "s/tight,//g" -e "s/freq//g" -e "s/__NAME__/$NEWNAME/g" $SCRIPTS_HOME//templates/template-pw91-HL.com > $NEWNAME.com
			  grep "^[NCHOSPF] " $NEWNAME.xyz  >> $NEWNAME.com
			  echo " " >> $NEWNAME.com
		  endif
		  ...
		  if($res == 1) then
			  ...
			  echo "g09 $NEWNAME.com" >> run-$jobnumber.pbs
		  else
			  echo "g09 $NEWNAME.com" >> run-$jobnumber.pbs
		  endif
