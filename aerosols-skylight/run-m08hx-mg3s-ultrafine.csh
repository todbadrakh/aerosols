...
foreach OLDNAME (`cat $TAG | awk 'BEGIN{ c = '$CUTOFF'}{if($7 < c) {print $1} else {}}'`)
  ...
  setenv FILE `echo $OLDNAME | awk -F"/" '{print $1}'| sed "s/.xyz//g"`
    ...
    echo "PASS"
      ...
      cp -f ../$FILE.xyz $NEWNAME.xyz
    endif

	  if( $RERUN ) then
		  echo "%nproc=10" > $NEWNAME.com
		  echo "%mem=24gb" >> $NEWNAME.com
      ...
      echo " " >> $NEWNAME.com
		  ...
		  echo "%mem=24gb" >> $NEWNAME.com
		  echo "%chk=$NEWNAME.chk" >> $NEWNAME.com
		  echo "# m08hx/gen scf(conver=10) geom(checkpoint,newdefinition) opt(tight,maxcycles=500) freq" >> $NEWNAME.com
		  echo " " >> $NEWNAME.com
		  echo "$NEWNAME" >> $NEWNAME.com
		  echo " " >> $NEWNAME.com
		  echo "0 1" >> $NEWNAME.com
      ...
      echo " " >> $NEWNAME.com
		  ...
		  sed -e "s/opt/scf(conver=10) opt/g" -e "s/__NAME__/$NEWNAME/g" $SCRIPTS_HOME/templates/template-m08hx-gen-ultrafine.com > $NEWNAME.com
		  grep "^[NCHOSPF] " $NEWNAME.xyz  >> $NEWNAME.com
		  echo " " >> $NEWNAME.com
      setenv H `grep H $NEWNAME.com | tail -n 1 | awk '{print $1}'`
      setenv C `grep C $NEWNAME.com | tail -n 1 | awk '{print $1}'`
	  ...
	  @ res = $count % $NJOBS
	  if($res == 1) then
		  sed -e "s/__JN__/$JOBNAME/g" -e "s/__NAME__/$NEWNAME/g" -e "s/__QUEUE__/$QUEUE/g" $SCRIPTS_HOME/templates/template-skylight-himem.slurm > run-$jobnumber.slurm
